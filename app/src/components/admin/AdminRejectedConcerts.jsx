...
const AdminRejectedConcerts = () => {
    ...
    // Load rejected concerts when component mounts
    useEffect(() => {
    ...
    const fetchRejectedConcerts = async () => {
        try {
            setLoading(true);
            setError('');
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch rejected concerts: ${response.status}`);
            }

            const data = await response.json();
            console.log('Rejected concerts:', data);

            // Set concerts data
            setRejectedConcerts(Array.isArray(data) ? data : []);
            setLoading(false);
        } catch (err) {
            console.error('Error fetching rejected concerts:', err);
            setError(`Failed to load rejected concerts: ${err.message}`);
            setLoading(false);
        }
    };
    ...
    const filteredConcerts = rejectedConcerts.filter(concert =>
    ...
    return (
        <div className="container mx-auto p-6">
            <h1 className="text-3xl font-bold mb-6 text-white">Rejected Concerts</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Left panel - Rejected concerts list */}
                <div className="lg:col-span-1 bg-gray-800 rounded-lg p-4 h-[calc(100vh-240px)] overflow-y-auto">
                    <h2 className="text-xl font-semibold text-white mb-4">
                        Rejected Concerts ({filteredConcerts.length})
                    </h2>

                    {filteredConcerts.length === 0 ? (
                        <div className="text-center py-10">
                            <p className="text-gray-400">No rejected concerts found.</p>
                        </div>
                    ) : (
                                ...
                                >
                                    ...
                                    {concert.approvalHistory && concert.approvalHistory.length > 0 && (
                                        <div className="mt-1 text-xs text-red-400">
                                            Rejected: {formatDate(concert.approvalHistory[concert.approvalHistory.length - 1].timestamp)}
                                        </div>
                                    )}
                                </div>
                            ))}
                    ...
                    {!selectedConcert ? (
                        ...
                        <>
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <span className="px-3 py-1 rounded-full text-sm bg-red-900/30 text-red-500">
                                    Rejected
                                </span>
                            </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    {selectedConcert.approvalHistory && selectedConcert.approvalHistory.length > 0 && (
                                        <>
                                            ...
                                            <div>
                                                <p className="text-gray-400 text-sm">Rejection Date</p>
                            ...
                            {selectedConcert.approvalHistory && selectedConcert.approvalHistory.length > 0 && (
                                <div className="mb-6">
                                    ...
                                    <div className="bg-red-900/20 border border-red-800/30 p-3 rounded text-white">
                                        {selectedConcert.approvalHistory[selectedConcert.approvalHistory.length - 1].message || "No reason provided"}
                                    </div>
                                </div>
                            )}
...
export default AdminRejectedConcerts;